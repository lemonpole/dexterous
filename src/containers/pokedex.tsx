...
function MoveSet( props: { speciesInfo: GraphQL.PokemonDetailsQuery }) {
  ...
  const [ pokemonGenerations, setPokemonGenerations ] = React.useState<typeof state['pokemonGenerations']>( [] );
  const [ selectedPokemonGeneration, setSelectedPokemonGeneration ] = React.useState<number>( 0 );
  ...
  const moveSetData = React.useMemo( () => {
    ...
    return uniqBy(
      props.speciesInfo
        .pokemon_v2_pokemonmove
        .filter( item => item.pokemon_v2_versiongroup?.pokemon_v2_generation?.id === selectedPokemonGeneration )
  ...
  return (
    <React.Fragment>
      <Select
        onChange={event => setSelectedPokemonGeneration( parseInt( event.target.value ) )}
        ...
        {pokemonGenerations.map( generation => (
          ...
          >
            {generation
              ...
              .map( versionGroup => versionGroup.pokemon_v2_versions
                .filter( version => !Constants.Application.IGNORED_GAMES.includes( version.name.toLowerCase() ) )
              ...
              <Tr key={move.id}>
                ...
                <Td>
                  <VStack align="flex-start">
                    <Text>{startCase( move.pokemon_v2_move?.name.replace( '-', ' ' ) )}</Text>
                    <HStack fontFamily="mono" fontSize="xs" width="full">
                      <Text width="50%">PWR: {move.pokemon_v2_move?.power || 'N/A'}</Text>
                      <Text>ACC: {move.pokemon_v2_move?.accuracy || 'N/A'}{!!move.pokemon_v2_move?.accuracy && '%'}</Text>
                    </HStack>
                  </VStack>
                </Td>
                <Td align="center">
  ...
  return (
    <React.Fragment>
      ...
      >
        ...
        <Stack width="full">
          <Heading as="h3" size="md">Move Set</Heading>
