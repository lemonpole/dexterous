...
export default function SpotlightImage(props: SpotlightImageProps) {
  const bgImageSrc = util.formatString(Constants.PokemonSpriteURLs.DEFAULT, [
    props.pokemonId?.toString() || '1',
  ]);
  const coverImageSrc = util.formatString(
    Constants.PokemonSpriteURLs.OFFICIAL_ARTWORK,
    [props.pokemonId?.toString() || '1']
  );

  // offset height if on mobile
  const baseHeight =
    props.height || Constants.Application.SPOTLIGHT_IMAGE_HEIGHT;
  const heightOffset = {
